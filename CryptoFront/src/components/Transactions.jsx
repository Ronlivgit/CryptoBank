        ...
        <table className='table text-center'>
            ...
            <tbody className=''>
                {props.txArray.map((item) => {
                    ...
                    const from = item.from === "0x0000000000000000000000000000000000000000" ? "Bank" : item.from;
                    const to = item.to === "0x0000000000000000000000000000000000000000" ? "Bank" : item.to;

                    const isIncome = item.from !== user.user.activeAddress && item.amount > 0
                    ...
                    return (
                        <tr key={item.timestamp} className='border text-white font-semibold text-center'>
                            <th className='border border-black/50'>{date}</th>
                            <td className='border border-black/50'>{item.operationType}</td>
                            <td className='border border-black/50'>
                                <span className='hoverable'>
                                    {fromBns || `${from.substring(0,5)}.... ${from.substring(from.length-5,from.length)}`}
                                    {fromBns && <span className='hover-text'>{from}</span>}
                                </span>
                            </td>
                            <td className='border border-black/50'>
                                <span className='hoverable'>
                                    {toBns || `${to.substring(0,5)}.... ${to.substring(to.length-5,to.length)}`}
